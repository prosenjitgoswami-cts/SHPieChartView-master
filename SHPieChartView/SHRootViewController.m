...
- (void)viewDidLoad
{
    ...
    CGFloat outerRadius = 250;
    CGFloat innerRadius = outerRadius - (lineWidth * 3 );
    
    CGFloat originYOnRactangleView  = 0 ;

    CGFloat  innerViewWidth  = 150 ;
    CGFloat innerViewHeight = sqrt(pow(innerRadius, 2) - pow(innerViewWidth + 20, 2));

    originYOnRactangleView = (innerRadius/2) - (innerViewHeight/2);
    
    NSDictionary *metrics = @{@"radius": @(outerRadius),
                              @"outerRadius": @(outerRadius),
                              @"innerRadius": @(innerRadius),
                              @"innerViewHeight": @(innerViewHeight),
                              @"innerViewWidth": @(innerViewWidth),
                              @"originYOnRactangleView" : @(originYOnRactangleView + (outerRadius -innerRadius)/2)};


    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-50-[contaonerView(radius)]" options:0 metrics:metrics views:_views]];
    
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-100-[contaonerView(radius)]" options:0 metrics:metrics views:_views]];
    

    {
        
    CircularPickerView *circularPickerView = [CircularPickerView new];
    circularPickerView.lineColor = [UIColor brownColor];
    circularPickerView.lineWidth = lineWidth;
        
        circularPickerView.bgColor = [UIColor greenColor];

        circularPickerView.minValue = 0;
        circularPickerView.maxValue = 100;
        circularPickerView.step = 1;
        circularPickerView.currentValue = 30;
    ...
    NSDictionary *_views = @{@"circularPickerView": circularPickerView,
        ...
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:[circularPickerView(outerRadius)]" options:0 metrics:metrics views:_views]];
        
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[circularPickerView(outerRadius)]" options:0 metrics:metrics views:_views]];
        ...
        [containerView addConstraint:[NSLayoutConstraint constraintWithItem:circularPickerView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:containerView attribute:NSLayoutAttributeCenterY multiplier:1 constant:0]];
        {
            ...
            CircularPickerView *circularPickerView = [CircularPickerView new];
            circularPickerView.lineColor = [UIColor purpleColor];
            circularPickerView.lineWidth = lineWidth;
            ...
            [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:[circularPickerView(innerRadius)]" options:0 metrics:metrics views:_views]];
            
            [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[circularPickerView(innerRadius)]" options:0 metrics:metrics views:_views]];
